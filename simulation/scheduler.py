...
AlarmData = collections.namedtuple('AlarmData', ('start_time', 'end_time',
                                                 'entry', 'is_random'))
...
def TunableDayAvailability():
    ...
    day_availability = TunableAvailableDays(
        description='Which days of the week to include',
        **day_availability_mapping)
...
class WeeklySchedule:
    ...
    def __init__(self,
        ...
        for entry in schedule_entries:
            for (start_time, end_time) in entry.get_start_and_end_times():
                ...
                self._schedule_entires.add(AlarmData(start_time, end_time,
                                                     entry, is_random))
    ...
    def time_until_next_scheduled_event(self,
        ...
        for alarm_data in self._schedule_entires:
            start_time = alarm_data.start_time
            end_time = alarm_data.end_time
            cur_time = current_date_and_time.time_till_timespan_of_week(
                start_time,
                optional_end_time=end_time if schedule_immediate else None,
                min_duration_remaining=min_duration_remaining)
    ...
    def get_schedule_times(self):
        ...
        for (start_time, end_time, _, _) in self._schedule_entires:
            busy_times.append((start_time.absolute_ticks(),
                               end_time.absolute_ticks()))
        return busy_times
        ...
        for this_time in busy_times:
            for other_time in other_busy_times:
                ...
                if starting_time_delta >= 0:
                    earlier_career_duration = other_time[END] - other_time[
                        START]
                else:
...
TunableWeeklyScheduleFactory = TunableFactory.create_auto_factory(
    WeeklySchedule)


class SituationScheduleEntry(ScheduleEntry):
    __qualname__ = 'SituationScheduleEntry'
    FACTORY_TUNABLES = {'situation': TunableReference(
        description=
...
TunableSituationScheduleEntry = TunableSingletonFactory.create_auto_factory(
    SituationScheduleEntry)


class SituationWeeklySchedule(WeeklySchedule):
    __qualname__ = 'SituationWeeklySchedule'
    FACTORY_TUNABLES = {'schedule_entries': TunableList(
        description=
        '\n            A list of event schedules. Each event is a mapping of days of\n            the week to a start_time and duration.\n            ',
        tunable=TunableSituationScheduleEntry())}


TunableSituationWeeklyScheduleFactory = TunableFactory.create_auto_factory(
    SituationWeeklySchedule)
