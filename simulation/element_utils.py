...
def _build_from_iterable(elem_iterable, sequence_wrapper=None):
    ...
    if sequence_wrapper is None:
        ...
        filtered_list = [sequence_wrapper(e)
                         for e in processed_list if e is not None]
...
def _build_with_finally(sequence):
    ...
    if final is not None and (inspect.isgeneratorfunction(final) or
                              not inspect.isroutine(final)):
...
def soft_sleep_forever():
    return return_true_wrapper(elements.RepeatElement(
        elements.SoftSleepElement(date_and_time.create_time_span(days=28))))
...
def with_callback(target_list, callback, sequence=None):
    ...
    return build_critical_section_with_finally(add_callback, sequence,
                                               remove_callback)
...
def do_all(*parallel_elements, thread_element_map=None):
    if not thread_element_map:
        ...
        all_elements = tuple(build_element(sequence)
                             for sequence in thread_element_map.values())
    else:
        all_elements = itertools.chain(parallel_elements, tuple(
            build_element(sequence)
            for sequence in thread_element_map.values()))
    all_elements = tuple(element
                         for element in all_elements if element is not None)
