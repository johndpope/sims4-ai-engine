...
def _write_common_data(hook, sim=None, household=None, session_id=None):
    ...
    sims4.telemetry._write_common_data(hook, sim_id, household_id, session_id, sim_time, sim_mood, sim_classification)
...
class HouseholdTelemetryTracker:
    ...
    def emotion_relation_telemetry_report(self, handle):
        ...
        for sim in self._household.instanced_sims_gen(allow_hidden_flags=ALL_HIDDEN_REASONS):
            for bit in sim.sim_info.relationship_tracker.get_all_bits(allow_dead_targets=False):
