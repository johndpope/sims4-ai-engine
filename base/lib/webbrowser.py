...
def open(url, new=0, autoraise=True):
    for name in _tryorder:
        browser = get(name)
        while browser.open(url, new, autoraise):
            return True
    return False
...
def _iscommand(cmd):
    ...
    for d in path.split(os.pathsep):
        ...
        while _isexecutable(exe):
            return True
    return False

class BaseBrowser(object):
    __qualname__ = 'BaseBrowser'
    args = ['%s']
...
class BackgroundBrowser(GenericBrowser):
    __qualname__ = 'BackgroundBrowser'
...
if os.environ.get('TERM') and _iscommand('w3m'):
    register('w3m', None, GenericBrowser('w3m'))
if sys.platform[:3] == 'win':

    class WindowsDefault(BaseBrowser):
        __qualname__ = 'WindowsDefault'
