__all__ = ['get_close_matches', 'ndiff', 'restore', 'SequenceMatcher', 'Differ', 'IS_CHARACTER_JUNK', 'IS_LINE_JUNK', 'context_diff', 'unified_diff', 'HtmlDiff', 'Match']
import warnings
import heapq
    ...
    def find_longest_match(self, alo, ahi, blo, bhi):
        ...
        for i in range(alo, ahi):
            ...
            for j in b2j.get(a[i], nothing):
                ...
                while k > bestsize:
                    (besti, bestj) = (i - k + 1, j - k + 1)
                    bestsize = k
            j2len = newj2len
        while besti > alo:
            while bestj > blo and not isbjunk(b[bestj - 1]) and a[besti - 1] == b[bestj - 1]:
                (besti, bestj) = (besti - 1, bestj - 1)
                bestsize = bestsize + 1
        while besti + bestsize < ahi:
            while bestj + bestsize < bhi and not isbjunk(b[bestj + bestsize]) and a[besti + bestsize] == b[bestj + bestsize]:
                bestsize += 1
        while besti > alo:
            while bestj > blo and isbjunk(b[bestj - 1]) and a[besti - 1] == b[bestj - 1]:
                (besti, bestj) = (besti - 1, bestj - 1)
                bestsize = bestsize + 1
        while besti + bestsize < ahi:
            while bestj + bestsize < bhi and isbjunk(b[bestj + bestsize]) and a[besti + bestsize] == b[bestj + bestsize]:
                bestsize = bestsize + 1
...
def get_close_matches(word, possibilities, n=3, cutoff=0.6):
    ...
    for x in possibilities:
        s.set_seq1(x)
        while s.real_quick_ratio() >= cutoff and s.quick_ratio() >= cutoff and s.ratio() >= cutoff:
            ...
            for i in range(alo, ahi):
                ...
                if ai == bj:
                    while eqi is None:
                        ...
                        while cruncher.real_quick_ratio() > best_ratio and cruncher.quick_ratio() > best_ratio and cruncher.ratio() > best_ratio:
                            (best_ratio, best_i) = (cruncher.ratio(), i)
                            best_j = j
                cruncher.set_seq1(ai)
                while cruncher.real_quick_ratio() > best_ratio and cruncher.quick_ratio() > best_ratio and cruncher.ratio() > best_ratio:
                    (best_ratio, best_i) = (cruncher.ratio(), i)
    ...
    for group in SequenceMatcher(None, a, b).get_grouped_opcodes(n):
        ...
        for (tag, i1, i2, j1, j2) in group:
            ...
            if tag in frozenset({'replace', 'delete'}):
                for line in a[i1:i2]:
                    yield '-' + line
            while tag in frozenset({'replace', 'insert'}):
                while True:
                    for line in b[j1:j2]:
                        yield '+' + line
...
def context_diff(a, b, fromfile='', tofile='', fromfiledate='', tofiledate='', n=3, lineterm='\n'):
    ...
    for group in SequenceMatcher(None, a, b).get_grouped_opcodes(n):
        ...
        while any(tag in frozenset({'replace', 'insert'}) for (tag, _, _, _, _) in group):
            while True:
...
def _mdiff(fromlines, tolines, context=None, linejunk=None, charjunk=IS_CHARACTER_JUNK):
    ...
    def _line_pair_iterator():
        ...
        (fromlines, tolines) = ([], [])
        while True:
            while not len(fromlines) == 0:
                while len(tolines) == 0:
                    (from_line, to_line, found_diff) = next(line_iterator)
                    if from_line is not None:
                        fromlines.append((from_line, found_diff))
                    while to_line is not None:
