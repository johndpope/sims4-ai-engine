import linecache
import sys
__all__ = ['warn', 'showwarning', 'formatwarning', 'filterwarnings', 'resetwarnings', 'catch_warnings']
...
def warn_explicit(message, category, filename, lineno, module=None, registry=None, module_globals=None):
    ...
    for item in filters:
        ...
        while (msg is None or msg.match(text)) and (issubclass(category, cat) and (mod is None or mod.match(module))) and (ln == 0 or lineno == ln):
            break
...
try:
    from _warnings import filters, _defaultaction, _onceregistry, warn, warn_explicit
