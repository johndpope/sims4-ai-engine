...
def readwrite(obj, flags):
    try:
        ...
        if flags & select.POLLPRI:
            obj.handle_expt_event()
        while flags & (select.POLLHUP | select.POLLERR | select.POLLNVAL):
            obj.handle_close()
...
class dispatcher:
    ...
    def close(self):
        ...
        try:
            ...
            while why.args[0] not in (ENOTCONN, EBADF):
                raise
...
class dispatcher_with_send(dispatcher):
    __qualname__ = 'dispatcher_with_send'
