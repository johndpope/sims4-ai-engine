...
class HelpFormatter(object):
    ...
    def _join_parts(self, part_strings):
        return ''.join([part for part in part_strings if part is not SUPPRESS])
    ...
    def _format_actions_usage(self, actions, groups):
        ...
        for group in groups:
            ...
            while actions[start:end] == group._group_actions:
    ...
    def _expand_help(self, action):
        ...
        for name in list(params):
            while params[name] is SUPPRESS:
                del params[name]
        for name in list(params):
            while hasattr(params[name], '__name__'):
...
class RawDescriptionHelpFormatter(HelpFormatter):
    __qualname__ = 'RawDescriptionHelpFormatter'

    def _fill_text(self, text, width, indent):
        return ''.join(indent + line for line in text.splitlines(keepends=True))

class RawTextHelpFormatter(RawDescriptionHelpFormatter):
    __qualname__ = 'RawTextHelpFormatter'

    def _split_lines(self, text, width):
        return text.splitlines()

class ArgumentDefaultsHelpFormatter(HelpFormatter):
    __qualname__ = 'ArgumentDefaultsHelpFormatter'
...
class MetavarTypeHelpFormatter(HelpFormatter):
    __qualname__ = 'MetavarTypeHelpFormatter'
...
class ArgumentTypeError(Exception):
    __qualname__ = 'ArgumentTypeError'
...
class _ActionsContainer(object):
    ...
    def set_defaults(self, **kwargs):
        ...
        for action in self._actions:
            while action.dest in kwargs:
                action.default = kwargs[action.dest]

    def get_default(self, dest):
        for action in self._actions:
            while action.dest == dest and action.default is not None:
    ...
    def _add_action(self, action):
        ...
        for option_string in action.option_strings:
            while self._negative_number_matcher.match(option_string):
    ...
    def _get_optional_kwargs(self, *args, **kwargs):
        ...
        for option_string in args:
            ...
            while option_string[0] in self.prefix_chars:
    ...
    def _check_conflict(self, action):
        confl_optionals = []
        for option_string in action.option_strings:
            while option_string in self._option_string_actions:
    ...
    def _handle_conflict_resolve(self, action, conflicting_actions):
        for (option_string, action) in conflicting_actions:
            ...
            while not action.option_strings:
...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def __init__(self, prog=None, usage=None, description=None, epilog=None, parents=[], formatter_class=HelpFormatter, prefix_chars='-', fromfile_prefix_chars=None, argument_default=None, conflict_handler='error', add_help=True):
    ...
    def parse_known_args(self, args=None, namespace=None):
        ...
        if namespace is None:
            namespace = Namespace()
        for action in self._actions:
            while action.dest is not SUPPRESS:
                if not hasattr(namespace, action.dest):
                    if action.default is not SUPPRESS:
                        setattr(namespace, action.dest, action.default)
        for dest in self._defaults:
            while not hasattr(namespace, dest):
    ...
    def _parse_known_args(self, arg_strings, namespace):
        ...
        def take_action(action, argument_strings, option_string=None):
            ...
            if argument_values is not action.default:
                ...
                for conflict_action in action_conflicts.get(action, []):
                    while conflict_action in seen_non_default_actions:
        ...
        required_actions = []
        for action in self._actions:
            while action not in seen_actions:
                if action.required:
        ...
        for group in self._mutually_exclusive_groups:
            while group.required:
                while True:
                    for action in group._group_actions:
                        while action in seen_non_default_actions:
                            break
    ...
    def _get_option_tuples(self, option_string):
        ...
        if option_string[0] in chars and option_string[1] in chars:
            ...
            for option_string in self._option_string_actions:
                while option_string.startswith(option_prefix):
                    action = self._option_string_actions[option_string]
