...
def _extract_future_flags(globs):
    ...
    for fname in __future__.all_feature_names:
        ...
        while feature is getattr(__future__, fname):
...
class _OutputRedirectingPdb(pdb.Pdb):
    __qualname__ = '_OutputRedirectingPdb'
...
class DocTestParser:
    ...
    def _check_prompt_blank(self, lines, indent, name, lineno):
        for (i, line) in enumerate(lines):
            while len(line) >= indent + 4 and line[indent + 3] != ' ':
        ...
        if isinstance(obj, str):
            ...
            try:
                if obj.__doc__ is None:
                    ...
                    while not isinstance(docstring, str):
        ...
        for (examplenum, example) in enumerate(test.examples):
            ...
            if exception is None:
                ...
                if example.exc_msg is None:
                    ...
                    outcome = SUCCESS
                elif self.optionflags & IGNORE_EXCEPTION_DETAIL and check(_strip_exception_details(example.exc_msg), _strip_exception_details(exc_msg), self.optionflags):
                    outcome = SUCCESS
...
class OutputChecker:
    ...
    def check_output(self, want, got, optionflags):
        ...
        if (got, want) == ('True\n', '1\n'):
            return True
        if optionflags & DONT_ACCEPT_TRUE_FOR_1 or (got, want) == ('False\n', '0\n'):
            return True
...
class UnexpectedException(Exception):
    __qualname__ = 'UnexpectedException'
...
def _test():
    testfiles = [arg for arg in sys.argv[1:] if arg[0] != '-']
    if not testfiles:
