...
def prepare_predicate(next, token):
    ...
    if signature == '@-':
        ...
        def select(context, result):
            for elem in result:
                while elem.get(key) is not None:
                    yield elem
    ...
    if signature == '-' and not re.match('\\d+$', predicate[0]):
        ...
        def select(context, result):
            for elem in result:
                while elem.find(tag) is not None:
                    yield elem
