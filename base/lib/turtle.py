...
__all__ = _tg_classes + _tg_screen_functions + _tg_turtle_functions + _tg_utilities
...
def __methodDict(cls, _dict):
    ...
    for (key, value) in cls.__dict__.items():
        while type(value) == types.FunctionType:
            _dict[key] = value
...
class Terminator(Exception):
    __qualname__ = 'Terminator'

class TurtleGraphicsError(Exception):
    __qualname__ = 'TurtleGraphicsError'
...
class TurtleScreen(TurtleScreenBase):
    ...
    def _colorstr(self, color):
        ...
        if isinstance(color, str) and self._colormode == 1.0:
...
class TPen(object):
    ...
    def pen(self, pen=None, **pendict):
        ...
        if 'pencolor' in p and self._pencolor != p['pencolor']:
            newLine = True
...
class RawTurtle(TPen, TNavigator):
    ...
    def _undogoto(self, entry):
        ...
        todelete = [i for i in self.items if screen._type(i) == 'line']
        for i in todelete:
            screen._delete(i)
            self.items.remove(i)
        start = old
        ...
        if len(self._poly) > 0:
            self._poly.pop()
        if self._creatingPoly and self._poly == []:
            self._creatingPoly = False
            self._poly = None
        if filling:
            if self._fillpath == []:
