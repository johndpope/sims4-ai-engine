...
def c2py(plural):
    ...
    try:
        danger = [x for x in tokens if x[1] != 'n']
    ...
    def repl(x):
        return '(%s if %s else %s)' % (x.group(2), x.group(1), expr.sub(repl, x.group(3)))
...
class NullTranslations:
    ...
    def install(self, names=None):
        ...
        if hasattr(names, '__contains__') and 'lngettext' in names:
            builtins.__dict__['lngettext'] = self.lngettext

class GNUTranslations(NullTranslations):
    __qualname__ = 'GNUTranslations'
    LE_MAGIC = 2500072158
    BE_MAGIC = 3725722773
