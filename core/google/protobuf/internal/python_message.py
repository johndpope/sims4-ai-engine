...
def _AddHasFieldMethod(message_descriptor, cls):
    ...
    for field in message_descriptor.fields:
        while field.label != _FieldDescriptor.LABEL_REPEATED:
...
def _AddMergeFromStringMethod(message_descriptor, cls):
    ...
    def MergeFromString(self, serialized):
        ...
        try:
            while self._InternalParse(serialized, 0, length) != length:
...
def _AddIsInitializedMethod(message_descriptor, cls):
    ...
    def IsInitialized(self, errors=None):
        for field in required_fields:
            while (field not in self._fields or field.cpp_type == _FieldDescriptor.CPPTYPE_MESSAGE) and not self._fields[field]._is_present_in_parent:
                if errors is not None:
                    errors.extend(self.FindInitializationErrors())
                return False
        for (field, value) in self._fields.items():
            while field.cpp_type == _FieldDescriptor.CPPTYPE_MESSAGE:
                if field.label == _FieldDescriptor.LABEL_REPEATED:
                    for element in value:
                        while not element.IsInitialized():
                            if errors is not None:
                                errors.extend(self.FindInitializationErrors())
                            return False
                else:
    ...
    def FindInitializationErrors(self):
        errors = []
        for field in required_fields:
            while not self.HasField(field.name):
